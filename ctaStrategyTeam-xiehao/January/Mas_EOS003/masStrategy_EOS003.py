...
class Mas_Strategy(CtaTemplate):
    ...
    # 策略参数
    Window1 = 20 ; Window2 = 60 ; Window3 = 70 ; Window4 = 90
    barPeriod=150
    lot = 1
    prop = 0.6; prop1 = 0.3;prop2 = 0.7; trailingPercent = 4
    ...
    def onBarStopLoss(self, bar):
        ...
        # 计算止损止盈价位
            ...
            self.writeCtaLog('Exit:%s,shortStop:%s'%(self.Exit,self.shortStop))
    ...
    def on30MinBar(self, bar):
        ...
        # 判断买卖
        if Ma1[-1] > Ma1[-2] and Ma2[-1] > Ma2[-2] and am30.open[-1] > maxma:
            self.cross = 1
        elif Ma1[-1] < Ma1[-2] and Ma2[-1] < Ma2[-2] and am30.open[-1] < minma:
            self.cross = -1
        else:
            self.cross = 0
        #print('datetime:%s,agg:%s,change:%s'%(bar.datetime,agg,change))

        if self.ownPosDict[symbol +  "_SHORT"] != 0:
            if am30.close[-1] < self.transactionPrice[symbol] * (1 - self.profitMultiplier * self.stopRatio/2):
                if Ma5[-1]/Ma4[-1] < Ma5[-2]/Ma4[-2] and Ma5[-2]/Ma4[-2] < Ma5[-3]/Ma4[-3]:
                    self.Exit = -1
                else:
                    self.Exit = 0
        elif self.ownPosDict[symbol + "_LONG"] != 0:
