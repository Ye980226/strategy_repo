...
class macd_Strategy(CtaTemplate):
    ...
    def __init__(self, ctaEngine, setting):
        ...
        super(macd_Strategy, self).__init__(ctaEngine, setting)
    ...
    def on15MinBar(self, bar):
        ...
        std = ta.STDDEV(am15.close,20)
        mean = ta.MA(std,50)*100
        ...
        if macd[-5] < 0 and macd[-2]<macd[-3]<macd[-4]<macd[-5] and macd[-1]>macd[-2]:
            ...
            self.cross[symbol] = 0
        # 金叉和死叉的条件是互斥
        if (self.cross[symbol] == 1 and self.trend[symbol] == 1 and mean[-1]<5):
            # 如果金叉时手头没有持仓，则直接做多
            if (self.posDict[symbol + "_LONG"] == 0) and (self.posDict[symbol + "_SHORT"] == 0):
        ...
        # 死叉和金叉相反
        elif (self.cross[symbol] == -1 and self.trend[symbol] == -1 and mean[-1]<5):
            if (self.posDict[symbol + "_LONG"] == 0) and (self.posDict[symbol + "_SHORT"] == 0):
