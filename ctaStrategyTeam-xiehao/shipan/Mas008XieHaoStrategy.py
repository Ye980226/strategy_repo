        ...
        # 洗价器
            ...
            if self.n<3:
                if (bar.close-self.transactionPrice[symbol])/self.transactionPrice[symbol]>=0.02:
                    self.buy(symbol, bar.close*1.015, self.fixsize*2^self.n,  levelRate=20)
                    self.n+=1
            ...
            if self.n<3:
                if (bar.close-self.transactionPrice[symbol])/self.transactionPrice[symbol]<=-0.02:
                    self.short(symbol,bar.close*0.995, self.fixsize*2^self.n,  levelRate=20)
                    self.n+=1
            if bar.close >= self.shortStop:
                ...
                self.n = 0
                self.flag = 1
                self.writeCtaLog('卖出价格%s,空头触发出场价格:%s,止损价格:%s' % (self.transactionPrice[symbol],bar.close,self.shortStop))
            elif bar.close < self.shortexit:
                self.cancelAll()
                self.cover(symbol, bar.close * 1.02, self.posDict[symbol + "_SHORT"],  levelRate=20)
                self.n = 0
                self.flag = 1
                self.writeCtaLog('卖出价格%s,空头触发出场价格:%s,止盈价格:%s' % (self.transactionPrice[symbol],bar.close, self.shortexit))
