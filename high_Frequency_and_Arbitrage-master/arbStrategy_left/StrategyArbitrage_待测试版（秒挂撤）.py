...
这一版使用的是秒挂单秒撤单逻辑，尝试更快速的挂撤单是否能够适用于更低的阈值
"""
...
class ArbitrageStrategy(CtaTemplate):
    ...
    def limitOrderOpen(self,order,mapDict,
        ...
        thisPrice = self.dealPrice(order)  #成交时为本次成交价，其它情况为0
    ...
    def limitCloseOrder(self,order,mapDict,
        ...
        if order.status == STATUS_NOTTRADED:
            self.cancelOrderDict[order.vtOrderID] = 60
    ...
    def onOrder(self, order):
        ...
        self.writeCtaLog('收到订单回报，orderDatetime:%s,localId:%s, price:%s, symbol:%s, direction：%s, offset:%s, status:%s'%(order.orderDatetime,
                        order.vtOrderID, order.price_avg,order.vtSymbol,order.direction,order.offset,order.status))
    ...
    def onTimer1000Ms(self,event):
        ...
        ### 首先撤掉所有的订单， 撤单被放在了onOrder中
        for orderID in list(self.cancelOrderDict):
            if self.cancelOrderDict[orderID] <= 60:
        ...
        for level in self.floorDict:
            ...
            rev_floor = oneDict['rev_floor']*oneDict['summean']
