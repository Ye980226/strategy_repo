...
class regRsiStrategy(CtaTemplate):
    ...
    def onBarPosition(self, bar):
        ...
        # antiMartingle
        if (self.ownPosDict[symbol + "_LONG"] == 0) and (self.ownPosDict[symbol + "_SHORT"] == 0):
            self.transactionPrice[symbol] = 0
            self.nPos[symbol] = 0
        # holdLong
        elif (self.ownPosDict[symbol+'_LONG']>0) and (self.nPos[symbol] < 1):
            if (self.transactionPrice[symbol]/bar.close-1)>= self.addPct:
        ...
        # holdShort
            ...
            if (bar.close/self.transactionPrice[symbol]-1)>= self.addPct:
