...
class macdAdxStrategy(CtaTemplate):
    ...
    # 策略参数
    barPeriod = 200
    adxPeriod = 55; adxMaType = 0; adxThreshold = 28
    fastPeriod = 30; slowPeriod = 60; signalPeriod = 24
...
# executeManagement--------------------------------------------------------
    ...
    # def onBar(self, bar):
        self.writeCtaLog('adxMacdStrategy####5S####ownposDict:%s####'%(self.ownPosDict))
    ...
    def onBarExecute(self, bar):
        symbol = bar.vtSymbol
#         print(self.adxCanTrade, self.priceDirection)
    ...
    def onTrade(self, trade):
        ...
        # 发送止盈单
        longFirstProfit = self.transactionPrice[symbol+'%s'%self.nPos]*(1+self.takeProfitFirstPct)
        longSecondProfit = self.transactionPrice[symbol+'%s'%self.nPos]*(1+self.takeProfitSecondPct)
        shortFirstProfit = self.transactionPrice[symbol+'%s'%self.nPos]*(1-self.takeProfitFirstPct)
        shortSecondProfit = self.transactionPrice[symbol+'%s'%self.nPos]*(1-self.takeProfitSecondPct)
