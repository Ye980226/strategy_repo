...
# executeManagement--------------------------------------------------------
    ...
    def fixedPctStopLoss(self, bar):
        symbol = bar.vtSymbol
        # 计算止损止盈价位
        lastOrder = self.transactionPrice[symbol]
        longStop = lastOrder*(1-self.stopLossPct)
        shortStop = lastOrder*(1+self.stopLossPct)
        # 持有多头
