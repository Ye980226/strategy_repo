...
########################################################################
class adxDiStairEosStrategy(CtaTemplate):
    className = 'diAdxStairEosStrategy'
    author = 'ChannelCMT'

    # 策略参数
    barPeriod = 200
    adxPeriod = 50; adxMaPeriod = 24; adxMaType = 2; adxThreshold = 20
    diPeriod = 80
    signalMaPeriod = 70; signalMaType = 0

    # 风控参数
    volPeriod = 40; highVolThrehold = 0.075
    trailingPct = 0.028; stopControlTime = 14
    takeProfitFirstPct = 0.045; takeProfitSecondPct = 0.075
    lot = 200

    # 仓位管理参数
    addPct = 0.003; addMultipler = 0.5
...
#     def onBar(self, bar):
        self.writeCtaLog('adxDiStairEosStrategy####5S####posDict:%s####'%(self.ownPosDict))
        self.onBarExitTimeControl(bar)
        EX = self.onBarExit(bar)
        if not EX:
            SL = self.onBarRiskControl(bar)
            if not SL:
                FL = self.volFilter(bar)
                if not FL:
    ...
    def onBarRiskControl(self, bar):
        ...
        # 持有多头仓位
            ...
            self.nChange = (self.intraTradeHigh/firstOrder-1)//self.trailingPct
        ...
        # 持有空头仓位
            ...
            self.nChange = -1* (self.intraTradeLow/firstOrder-1)//self.trailingPct
            changePrice = firstOrder*self.nChange*self.trailingPct
...
#          (minVol <= lowFilterRange) or
        highFilterRange = am5.close[-1]*self.highVolThrehold
        if(maxVol >= highFilterRange):
    ...
    def takeProfit(self, trade):
        ...
        if trade.offset == OFFSET_OPEN:
            if trade.direction == DIRECTION_LONG:
                self.sellClose(symbol, longFirstProfit, 2*trade.volume//3)
                self.sellClose(symbol, longSecondProfit, trade.volume-2*trade.volume//3)
                self.writeCtaLog('long### tp1:%s, tp2:%s'%(longFirstProfit, longSecondProfit))
            elif trade.direction == DIRECTION_SHORT:
                self.coverClose(symbol, shortFirstProfit,2*trade.volume//3)
                self.coverClose(symbol, shortSecondProfit,trade.volume-2*trade.volume//3)
