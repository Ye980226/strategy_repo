...
########################################################################
class adxMacdBtcStrategy(CtaTemplate):
    className = 'adxMacdBtcStrategy'
    author = 'ChannelCMT'
    ...
    # 风控参数
    trailingPct = 0.028; stopControlTime = 4
    volPeriod = 55; lowVolThrehold = 0.0018
    takeProfitFirstPct = 0.035; takeProfitSecondPct = 0.055
    lot = 2

    # 仓位管理参数
    addPct = 0.003; addMultipler = 0.8
...
#     def onBar(self, bar):
        self.writeCtaLog('adxMacdBtcStrategy####5S####ownposDict:%s####'%(self.ownPosDict))
    ...
    def onBarRiskControl(self, bar):
        ...
        # 持有多头仓位
            ...
            if bar.low <= self.longStop*1.002:
                self.sellCheckExtend(bar)
                stopLoss = True
        ...
        # 持有空头仓位
            ...
            if bar.high >= self.shortStop*0.998:
                self.coverCheckExtend(bar)
                stopLoss = True
    ...
    def onBarExecute(self, bar):
        symbol = bar.vtSymbol
        if (self.adxCanTrade == 1) and (self.filterCanTrade == 1):
