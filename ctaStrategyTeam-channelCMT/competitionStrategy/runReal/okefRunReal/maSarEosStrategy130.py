...
########################################################################
class maSarEosStrategy(CtaTemplate):
    className = 'maSarEosStrategy'
    author = 'ChannelCMT'

    # 策略参数
    barPeriod = 400
    maPeriod = 270; maType = 0
    sarAcceleration = 0.0045
    volPeriod = 40; lowVolThrehold = 0.002; highVolThrehold = 0.045

    # 风控参数
    lot = 600

    # 仓位管理参数
    addPct = 0.02; addMultipler = 0.3
    ...
    paramList = [
                 ...
                 'sarAcceleration',
                 'volPeriod',
                 'lowVolThrehold','highVolThrehold',
                 'atrPeriod','atrMultipler',
...
#     def onBar(self, bar):
    ...
    def volFilter(self, bar):
        ...
        if (minVol <= lowFilterRange) or (maxVol >= highFilterRange):
    ...
    def onBarExit(self, bar):
        ...
        if bar.low<maClose[-1]:
            if (self.ownPosDict[symbol + "_LONG"] > 0) :
                self.sellCheckExtend(bar)
                Exit = True
        elif bar.high>maClose[-1]:
    ...
    def onBarExecute(self, bar):
        ...
        if (bar.close>sar[-1]) and (am5.close[-2]<sar[-2]):
            self.priceDirection = 1
        elif (bar.close<sar[-1]) and (am5.close[-2]>sar[-2]):
            self.priceDirection = -1
        else:
            self.priceDirection = 0
        self.writeCtaLog('bar.close:%s, SAR: %s'%(bar.close, sar[-2:]))
