...
########################################################################
class adxBBandFakeBreakEosStrategy(CtaTemplate):
    className = 'adxBBandFakeBreakEosStrategy'
    author = 'ChannelCMT'

    # 策略参数
    barPeriod = 400
    adxPeriod = 20; adxThreshold = 25
    bBandPeriod = 50; bandStd1 = 2; bandStd2 = 4
    maxHappenBar = 30

    # 风控参数
    trailingPct = 0.02; stopControlTime = 5
#     takeProfitFirstPct = 0.04; takeProfitSecondPct = 0.065
    ...
    # 信号变量
    adxCanTrade = 0
    breakHappen = 0; happenBreakBar = 0
    stopLossControl = 0
...
#         self.onBarPosition(bar)
    ...
    def onBarHappen(self, bar):
        if self.breakHappen!=0:
            self.happenBreakBar+=1
        elif self.breakHappen==0:
            self.happenBreakBar = 0
        if self.happenBreakBar>=self.maxHappenBar:
            self.breakHappen = 0
            self.happenBreakBar = 0
    ...
    def onBarExecute(self, bar):
        ...
        bBandUp1, bBandMid1, bBandDn1 = ta.BBANDS(am15.close, self.bBandPeriod, self.bandStd1,self.bandStd1,0)
        bBandUp2, bBandMid2, bBandDn2 = ta.BBANDS(am15.close, self.bBandPeriod, self.bandStd2,self.bandStd2,0)

        breakUp2 = (am15.high[-2]<=bBandUp2[-2]) and (am15.high[-1]>bBandUp2[-2])
        breakDn2 = (am15.low[-2]>=bBandDn2[-2]) and (am15.low[-1]<bBandDn2[-2])
        ...
        breakUpBack = (self.breakHappen==1) and (am15.close[-1]<=bBandUp1[-1])
        breakDnBack = (self.breakHappen==-1) and (am15.close[-1]>=bBandDn1[-1])

#         print('breakHappen: %s'%(self.breakHappen), 'happenBreakBar:%s'%(self.happenBreakBar))
        if (self.adxCanTrade == 1):
            if (breakUpBack) and (self.ownPosDict[symbol + "_LONG"] == 0):
...
#                 if (self.stopLossControl == 0):
            ...
            elif (breakDnBack) and (self.ownPosDict[symbol + "_SHORT"] == 0):
#                 if self.stopLossControl == 1:
#                     self.stopLossControl = 0
#                 if (self.stopLossControl == 0):
                if (self.ownPosDict[symbol + "_LONG"] == 0):
