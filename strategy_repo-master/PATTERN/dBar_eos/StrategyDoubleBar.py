...
########################################################################
class StrategyTest(OrderTemplate):
    ...
    def entrySignal(self,bar):
        ...
        for candleType, candlePeriod in zip(self.candleTypes, self.candlePeriods):
            ...
            if TF:
                ...
                if vi.volumeSpike(self.volumeNperiod,self.volumeStdMultuper)[-1] and \
                    not vi.volumeSpike(self.volumeNperiod,self.volumeStdMultuper)[-2]:
                    signal3 = True
                else:
                    signal3 = False
