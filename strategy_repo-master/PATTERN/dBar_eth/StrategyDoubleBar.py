...
########################################################################
class StrategyDoubleBar(OrderTemplate):
    className = 'StrategyDoubleBar'
    author = 'zong'
    ...
    def onBar(self,bar):
        ...
        self.matlog['volume'].append(bar.volume)
        self.matlog['close'].append(bar.close)
    ...
    def entryOrder(self, entrySignal, bar):
        ...
        for candleType, signal in entrySignal.items():
            if signal == 1 and buy:
                ...
                self.sendbuyOrder(bar,bar.close,volume,1,market=True)
                self.sendbuyOrder(bar,bar.close-agap*self.secondMove,volume,14)
                ...
                self.sendShortOrder(bar,bar.close,volume,1,market=True)
                self.sendShortOrder(bar,bar.close+agap*self.secondMove,volume,14)
    ...
    def sendbuyOrder(self,bar,price,volume,waitmins,market=False):
        ...
        for vtOrderID in tlo.vtOrderIDs:
            ...
            self.orderDict[vtOrderID] = {
                'price':price,
                'firstmove':max(self.takeProfit,abs(bar.close-bar.open)/bar.open)
                # 'firstmove':0.01
            }
    ...
    #     super().onTrade(trade)
    #     self.writeCtaLog('onTrade发生了成交，方向：%s, 开平:%s, 成交价:%s'%(trade.direction, trade.offset, trade.price))
    ...
    #             self.setAutoExit(op, trade.price*(1-self.stopLoss),None)
    #         else:
    #             self.setAutoExit(op, trade.price*(1+self.stopLoss))
