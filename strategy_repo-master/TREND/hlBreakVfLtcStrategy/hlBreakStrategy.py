...
class hlBreakStrategy(OrderTemplate):
    ...
    def strategy(self, bar):
        ...
        # 根据出场信号出场
        highExitBand, lowExitBand = self.exitSignal(envPeriod, signalPeriod)
        if len(highExitBand) and len(lowExitBand):
            self.exitOrder(bar, highExitBand, lowExitBand)
    ...
    def exitOrder(self, bar, highExitBand, lowExitBand):
