...
from hlBreakDsSignalClass import hlBreakDsSignal

########################################################################
class hlBreakDsStrategy(OrderTemplate):
    className = 'hlBreakDs'
    author = 'ChannelCMT'
    ...
    def on5MinBar(self, bar):
        ...
        if engineType != 'backtesting':
            ...
            self.notifyPosition('longVolume', longVolume, 'ChannelPos')
            self.notifyPosition('shortVolume', shortVolume, 'ChannelPos')
    ...
    def entryOrder(self, bar, entrySignal):
        ...
        lotSize = self.lot * self.lotMultipler//2
        ...
        if entrySignal ==1:
            if not (self.orderDict['orderFirstLongSet'] or self.orderDict['orderSecondLongSet']):
                ...
                stepOrder1 = self.makeStepOrder(ctaBase.CTAORDER_BUY, bar.vtSymbol, buyExecute, lotSize, orderPos, self.totalSecond, self.stepSecond)                 
                orderID1 = stepOrder1.parentID
                self.orderDict['orderFirstLongSet'].add(orderID1)
                self.orderLastList.append(orderID1)
                # 第二单
                stepOrder2 = self.makeStepOrder(ctaBase.CTAORDER_BUY, bar.vtSymbol, buyExecute, lotSize, orderPos, self.totalSecond, self.stepSecond)                 
            ...
            if not (self.orderDict['orderFirstShortSet'] or self.orderDict['orderSecondShortSet']):
                ...
                # 第一单
                stepOrder1 = self.makeStepOrder(ctaBase.CTAORDER_SHORT, bar.vtSymbol, shortExecute, lotSize, orderPos, self.totalSecond, self.stepSecond)
                orderID1 = stepOrder1.parentID                
                self.orderDict['orderFirstShortSet'].add(orderID1)
                self.orderLastList.append(orderID1)
                # 第二单
                stepOrder2 = self.makeStepOrder(ctaBase.CTAORDER_SHORT, bar.vtSymbol, shortExecute, lotSize, orderPos, self.totalSecond, self.stepSecond)
                orderID2 = stepOrder2.parentID                
