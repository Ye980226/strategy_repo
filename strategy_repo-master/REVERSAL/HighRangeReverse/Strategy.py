...
########################################################################
class StrategyHighRangeRev(OrderTemplate):
    className = 'StrategyHighRangeRev'
    author = 'Rich'
    ...
    def entryOrder(self, sig, bar, stoploss):
        if sig > 0:
            ...
            for orderID in tlo.vtOrderIDs:
                ...
                op = self._orderPacks[orderID]
                self.setAutoExit(op, stoploss, bar.close * (1 + self.takeProfit))  # 设置止盈止损
        elif sig < 0:
            ...
            for orderID in tlo.vtOrderIDs:
                ...
                op = self._orderPacks[orderID]
                self.setAutoExit(op, stoploss, bar.close * (1 - self.takeProfit))
