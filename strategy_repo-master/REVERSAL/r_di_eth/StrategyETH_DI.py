...
class Strategy_signalDI(OrderTemplate):
    ...
    def __init__(self, ctaEngine, setting):
        ...
        ### 和指标相关的全局变量
        self.Filter = False
        self.buyClose, self.shorClose = datetime(2000,1,1), datetime(2000,1,1)
    ...
    def entrySignal(self,bar):
        ...
        if TF:
            Filter, _ = dg.ADXget(amfilter,self.adxPara)        
            self.Filter = Filter
        else:
            Filter = self.Filter
        ...
        if TF:
            atr = ta.ATR(amSignal.high,amSignal.low,amSignal.close,self.atrPeriod)[-1]/bar.close
            # if atr<self.atrShield:
            #     self.DIpara['DImaPeriod'] = 10
            #     self.DIpara['DIshield'] = 4*self.DIshield
            # else:
            #     self.DIpara['DImaPeriod'] = 1
            #     self.DIpara['DIshield'] = self.DIshield
            Signal, DI, _ = dg.DIget(amSignal, self.DIpara)
        ...
        ### 综合信号下单
        if Signal==1 and buy1 and buy2 and self.Filter and \
        (bar.datetime-self.buyClose).total_seconds()/60 > 60:
            self.writeCtaLog('策略判断出现多信号，买入开仓')
        elif Signal==-1 and short1 and short2 and self.Filter and \
        (bar.datetime-self.shorClose).total_seconds()/60 > 60:
