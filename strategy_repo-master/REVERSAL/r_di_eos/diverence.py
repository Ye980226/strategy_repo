...
class DiverGenceGet():
    ...
    def DIget(self,barArray, paraDict):
        ...
        # def retRemoveMaxBar(barArray,period):
        #     """
        #     计算最近period的收益，但会去掉period中变动幅度最大的一根bar
        #     """ 
        #     if len(barArray.open)<period:
        #         raise ValueError('输入的period:%s大于bar的长度:%s'%(len(barArray.open),period))
        #     barMove = (barArray.open - barArray.close)[-period:]
        #     maxbarMove = np.max(barMove)
        #     price1 = barArray.close[-1]
        #     price0 = barArray.close[-period] + maxbarMove
        #     return (price1-price0)/price0        

        slowSign = slowMom[-1]
        #### 考虑计算slowMom[-1]是否去除掉变动幅度最大的一根bar
        # slowSign = retRemoveMaxBar(barArray,slowPeriod)
        
        slowStd = ta.STDDEV(pctChange, slowPeriod)
        DI = (fastMom*slowMom)/(slowStd*slowStd)    
