...
########################################################################
class StrategyEOSGridV12(OrderTemplate):
    className = 'StrategyEOSGridV12'
    author = 'Rich'
    ...
    def onBar(self, bar):
        ...
        if engineType == 'backtesting':
            ...
            self.waitForCloseMaxPos = self.gridLevelMaxPos * self.gridLevel / 2
    ...
    def moveToWaitForClose(self):
        for i in range(1, self.gridLevel + 1):
            shortLevelPos = self.getLevelPos(i) # 空头未平仓量
            if shortLevelPos > 0 and shortLevelPos + self.getLevelPos("short", appendix="waitForClose") < self.waitForCloseMaxPos: # 有移动空头仓位的空间
                self.orderDict["waitForClose"]["short"] = self.orderDict["waitForClose"]["short"] | self.orderDict["gridLevel"][i]
                self.orderDict["gridLevel"][i] = set()

            buyLevelPos = self.getLevelPos(-i) # 多头未平仓量
            if buyLevelPos > 0 and buyLevelPos + self.getLevelPos("buy", appendix="waitForClose") < self.waitForCloseMaxPos: # 有移动多头仓位的空间
    ...
    def calEnv(self, period="1h"):
        ...
        if barPrepared:
            ...
            # 允许开仓,更新挂单中枢价格,移除旧中枢尚未出场的仓位
            self.canOpen = True
            old_grid0 = self.grid0
            self.grid0 = longMA[-self.continuousCalmHours:].mean()
            if abs(old_grid0/self.grid0 - 1) > self.grid0MoveLimit:
