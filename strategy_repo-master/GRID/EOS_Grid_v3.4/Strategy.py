...
########################################################################
class StrategyEOSGridV34(OrderTemplate):
    className = "StrategyEOSGridV34"
    author = 'Rich'
    ...
    paramList = [
        ...
        "volume_range", # 仓位衰减条件-成交量范围
        "para_a1", # 仓位衰减条件-系数
        "para_a2",
        "para_a3",
        "para_a4",
        "para_a5",
        "meanVolumeBarLens", # 仓位衰减条件-近期平均成交量
        "dayLen", # 仓位衰减条件-时间范围
    ]
    ...
    def __init__(self, ctaEngine, setting):
        ...
        self.start = None # 可以挂网格单的开始时间
        self.lot = self.initlot # 初始化下单手数
    ...
    def onBar(self, bar):
        ...
        if engineType == 'backtesting':
            ...
            self.checkOnPeriodStart(bar)
            # 回测时的下单手数按此方法调整
            self.initlot = round(100 / bar.close, 4)
            self.gridLevelMaxPos = self.initlot * 10
        ...
        self.strategy(bar,
                      ...
                      decayPosPeriod=self.timeframeMap["decayPosPeriod"],
    ...
    def strategy(self,
        ...
        # 根据网格持续交易时长衰减仓位
        self.decayBasePos(decayPosPeriod)
    ...
    def decayBasePos(self, period):
        am = self.getArrayManager(self.symbol, period)
        if am.inited:
            if (self.canOpen and self.start is None) or not self.canOpen:
                self.start = pd.to_datetime(am.datetime[-1])
            if self.canOpen:
                # 根据近期成交量和网格可开仓延续时间，衰减仓位
                recent_meanV = am.volume[-self.meanVolumeBarLens:].mean()
                if recent_meanV <= self.volume_range[0]:
                    para_a = self.para_a1
                elif recent_meanV <= self.volume_range[1]:
                    para_a = self.para_a2
                elif recent_meanV <= self.volume_range[2]:
                    para_a = self.para_a3
                elif recent_meanV <= self.volume_range[3]:
                    para_a = self.para_a4
                else:
                    para_a = self.para_a5
                decayProp = para_a * (pd.to_datetime(am.datetime[-1]) - pd.to_datetime(self.start)).total_seconds() / (
                            24 * 60 * 60) / self.dayLen
                decayProp = max(-pow(decayProp, 2) + 1, 0)
                if decayProp == 0: # 衰减比例到0，则不可再开仓
                    self.canOpen = False
                    self.lot = 0
                else:
                    self.lot = int(self.initlot * decayProp) if self.getEngineType() != 'backtesting' else self.initlot * decayProp
                    self.gridLevelMaxPos = self.lot * 10 #TODO 回头考虑改一下 不要在这里硬编码
                    self.waitForCloseMaxPos = self.gridLevelMaxPos * len(self.gridlist) / 2 #TODO 回头考虑改一下 不要在这里硬编码
    ...
    def calEnv(self, period="1h"):
        ...
        if barPrepared:
            ...
            K = ta.LINEARREG_SLOPE(EMA, self.slopeLen)[-1]
