...
def run_tasks(pardir = None, test_run = False):
    ...
    for setting in settings:
        ...
        if test_run:
            end = datetime.strptime(setting['ENGINE_SETTINGS']['setEndDate'],"%Y%m%d %H:%M")
            start = end-timedelta(months= 1)
            setting['ENGINE_SETTINGS']['setStartDate'] = start.strftime("%Y%m%d %H:%M")
        ...
        for task in setting['TASK_LISTS']:
            ...
            print(STRATEGY_SETTING['strategy_filename']," : start ", task['task_name'])
            opt_result = start_optimize_n_output(engine, task, param_mode, opt_engine_setting, folderName, strategy, setting["OPT_TARGET"])
        with open(f'{folderName}/opt_result_summary.json', "w", encoding="utf-8") as f:
            json.dump(opt_result, f, indent=4)
        print(STRATEGY_SETTING['strategy_filename'], ": tasks finished ****")
