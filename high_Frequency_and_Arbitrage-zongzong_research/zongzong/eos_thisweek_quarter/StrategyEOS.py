...
确定一个最短持仓时间 1min
确定一个最长持仓时间 3hour，
随着持仓时间的推移，self.closespread逐渐减小。
...
class FishingNoramlStrategy(CtaTemplate):
    ...
    def __init__(self, ctaEngine, setting):
        ...
        self.posmorning = float(setting['posSize'])
...
#         self.saveSyncData()
    ...
    def onHFBar(self,bar):
        ...
        if (spread > self.miu_short + self.spreadBand) and len(self.limitOrderDict.keys()) == 0 and abs(self.pos)<self.maxpos and self.stopLost == 0:
            ### 在两个不同的tick位置排单
            for i in [0,self.mintick,2*self.mintick]:
        ...
        if self.pos > 0 and (datetime.now()-self.opentime).total_seconds()>30 and len(self.closeOrderDict.keys()) == 0 and self.stopLost == 0:
            ...
            if midActive - midPassive < self.miu_short - 1.5*self.gap or (datetime.now()-self.opentime).total_seconds()/60>240:
                self.stopLost = 1
                self.cancelAll()
                self.writeCtaLog('主动多头被动空头止损,sad')
            else:
                # 出场时控制因为对手单量过少带来的滑点
        ...
        if self.pos < 0 and (datetime.now()-self.opentime).total_seconds()>30 and len(self.closeOrderDict.keys()) == 0 and self.stopLost == 0:
            ...
            if midActive - midPassive > self.miu_short + 1.5*self.gap or (datetime.now()-self.opentime).total_seconds()/60>240:
                self.stopLost = 1
                self.cancelAll()
                self.writeCtaLog('主动空头被动多头止损,sad')                    
            else:
                # 出场时控制因为对手单量过少带来的滑点
                activeTick = self.tickBufferDict[self.activeSymbol][-1]
    ...
    def onOrder(self, order):
        ...
        if order.vtOrderID in self.limitOrderDict.keys():
            ...
            if order.status == STATUS_CANCELLED:
                ...
                if order.vtSymbol == self.passiveSymbol:
                    ##### 打印成交是对应品种的tick信息
                    tick = self.tickBufferDict[order.vtSymbol][-1]
                    pricelist = [tick.bidPrice5,tick.bidPrice4,tick.bidPrice3,tick.bidPrice2,tick.bidPrice1,tick.askPrice1,tick.askPrice2,tick.askPrice3,tick.askPrice4,tick.askPrice5]
                    volumelist = [tick.bidVolume5,tick.bidVolume4,tick.bidVolume3,tick.bidVolume2,tick.bidVolume1,tick.askVolume1,tick.askVolume2,tick.askVolume3,tick.askVolume4,tick.askVolume5]
                    self.writeCtaLog(u'tickPriceList:%s, volumeList:%s'%(pricelist,volumelist))
            ...
            # 变动的订单是市价追单
            if order.status == STATUS_REJECTED and order.totalVolume != 0:
                ####市价追单再发一遍
                if order.direction == DIRECTION_LONG and order.offset == OFFSET_OPEN:
        ...
        if order.status == STATUS_CANCELLED and self.stopLost == 1:
            self.cover(self.passiveSymbol,order.price,self.posDict[self.passiveSymbol+'_SHORT'],priceType = PRICETYPE_MARKETPRICE,levelRate = self.levelRate)
            self.cover(self.activeSymbol,order.price,self.posDict[self.activeSymbol+'_SHORT'],priceType = PRICETYPE_MARKETPRICE,levelRate = self.levelRate)
            self.sell(self.passiveSymbol,order.price,self.posDict[self.passiveSymbol+'_LONG'],priceType = PRICETYPE_MARKETPRICE,levelRate = self.levelRate)
            self.sell(self.activeSymbol,order.price,self.posDict[self.activeSymbol+'_LONG'],priceType = PRICETYPE_MARKETPRICE,levelRate = self.levelRate)
