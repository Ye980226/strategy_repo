...
class FishingNoramlStrategy(CtaTemplate):
    ...
    def onInit(self):
        ...
        self.datetime = {self.activeSymbol:datetime.now(),self.passiveSymbol:datetime.now()}
    ...
    def onStart(self):
        ...
        elif self.posDict[self.activeSymbol+'_SHORT']>0:
            self.pos = -self.posDict[self.activeSymbol+'_SHORT']
...
#         self.saveSyncData()
    ...
    def onBar(self, bar):
        ...
        # self.writeCtaLog('on1MinBarAHA, symbol:%s, datetime:%s'%(bar.vtSymbol,bar.datetime))        
        #每分钟记录输出的spread：
        if bar.vtSymbol == self.activeSymbol:
            self.writeCtaLog('on1MinBar, self.spread:%s,self.trade：%s , pos:%s'%(self.tickBufferDict[self.activeSymbol][-1].lastPrice-self.tickBufferDict[self.passiveSymbol][-1].lastPrice,self.trading,self.pos))
            self.spreadBuffer.append(self.tickBufferDict[self.activeSymbol][-1].lastPrice-self.tickBufferDict[self.passiveSymbol][-1].lastPrice)
        ...
        if self.trading == True:
            if Time.weekday() == 4 and Time.hour == 15 and Time.minute > 45:
                self.writeCtaLog('临近交割期策略自动停止')
                self.ctaEngine.trading = False
        else:
            if Time.weekday() == 4 and Time.hour == 18 and Time.minute > 1:
                self.writeCtaLog('交割结束2小时15分钟后策略重新启动')
                self.writeCtaLog.trading = True
    ...
    def on5MinBar(self,bar):
        ...
        if self.pos != 0:
            # 持仓时间超过1小时，更改出场参数
            if (datetime.now()-self.opentime).total_seconds()/60 > 60 and (datetime.now()-self.opentime).total_seconds()/60 <= 120:
                self.spreadclose = self.spreadclosenew 
                self.writeCtaLog('持仓超过1小时，更改selfclose参数')
                # self.mail(u'% 持仓超过1小时了'%self.name)
            elif  (datetime.now()-self.opentime).total_seconds()/60 > 120 and (datetime.now()-self.opentime).total_seconds()/60 <= 180:
                self.miu_short = self.miu_short_list[-1]
                self.writeCtaLog('持仓超过2小时，更改miu_short参数')
                # self.mail(u'% 持仓超过2小时了'%self.name)
            elif  (datetime.now()-self.opentime).total_seconds()/60 > 180:
                self.stopLost = 1
                self.writeCtaLog('持仓超过3小时，强行平仓出场') 
                self.opentime = datetime.now()
        self.putEvent()
