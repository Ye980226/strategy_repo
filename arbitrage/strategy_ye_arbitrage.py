...
class Strategy_Arbitrage(CtaTemplate):
    ...
    def onInit(self):
        ...
        self.offset_directionCatchOrderMap={("开仓","多"):self.catchShort,("开仓","空"):self.catchBuy,("平仓","多"):self.catchSell,("平仓","空"):self.catchCover}
        # 创建K线合成器对象
        self.bgDict = {
        ...
        self.floor_infoMap={floor:{"pos":0,"isNormal":False,"isEnter":False} for floor in self.floors}
        ...
        self.floor_tickRate={floor:out_extend for floor,out_extend in zip(self.floors,self.out_extend)}
        self.floor_priceVol={floor:in_extend for floor,in_extend in zip(self.floors,self.in_extend)}
        self.floor_volume={floor:volume for floor,volume in  zip(self.floors,self.base_unit)}
        self.open_closeGap={floor:floor-profit for floor,profit in zip(self.floors,self.profits)}
        ...
        miuSpreadMax=max((abs(self.miu_short-self.miu),abs(self.miu_long-self.miu)))
        ...
        self.writeCtaLog("activeSymbol的bar.close数组的长度%s"%len(self.barBufferDict[self.passiveSymbol]))
        self.writeCtaLog("barSpreadBuffer的长度为:%s"%(len(self.barSpreadBuffer)))
        miuSpreadMax=max((abs(self.miu_short-self.miu),abs(self.miu_long-self.miu)))
        if miuSpreadMax>self.gap_base:
    ...
    def catchBuy(self,order):
        ...
        activeID=self.buy(self.activeSymbol,self.midActiveBuffer[-1]*1.02,volume,levelRate=10,priceType=PRICETYPE_LIMITPRICE)[0]
        passiveID=order.vtOrderID#只维护开仓时的passiveID
        self.active_passiveMap[activeID]=passiveID
            
        self.writeCtaLog("市价追开空"+str(self.tickBufferDict[self.activeSymbol][-1].bidPrice1)+" "+"volume:"+str(volume))
    ...
    def catchShort(self,order):
        ...
        self.passive_volume[order.vtOrderID]=order.tradedVolume
        self.order_priceMap[order.vtOrderID]=order.price_avg#在开仓时保留下价格
        activeID=self.short(self.activeSymbol,self.midActiveBuffer[-1]*0.98,volume,levelRate=10,priceType=PRICETYPE_LIMITPRICE)[0]
        passiveID=order.vtOrderID #只维护开仓时的passiveID
        self.active_passiveMap[activeID]=passiveID
        
        
        self.writeCtaLog("市价追开多"+str(self.tickBufferDict[self.activeSymbol][-1].askPrice1)+" "+"volume:"+str(volume))

    # -----------------------------------------------------------------------------
    def catchSell(self,order):
        passiveID=self.close_openIdMap[order.vtOrderID]#平仓时映射到开仓时的vtOrderID
        volume=order.tradedVolume-self.passive_volume[passiveID]
        self.passive_volume[passiveID]=order.tradedVolume
        activeID=self.sell(self.activeSymbol,self.midActiveBuffer[-1]*0.98,volume,levelRate=10,priceType=PRICETYPE_LIMITPRICE)[0]
        self.active_passiveMap[activeID]=passiveID
        if order.status=="全部成交":
            del self.close_openIdMap[order.vtOrderID]
        self.writeCtaLog("市价追平多"+str(self.tickBufferDict[self.activeSymbol][-1].askPrice1)+" "+"volume:"+str(volume))
    ...
    def catchCover(self,order):
        ...
        volume=order.tradedVolume-self.passive_volume[passiveID]
        self.passive_volume[passiveID]=order.tradedVolume
        activeID=self.cover(self.activeSymbol,self.midActiveBuffer[-1]*1.02,volume,levelRate=10,priceType=PRICETYPE_LIMITPRICE)[0]
        self.active_passiveMap[activeID]=passiveID
        if order.status=="全部成交":
            del self.close_openIdMap[order.vtOrderID]
        self.writeCtaLog("市价追平空"+str(self.tickBufferDict[self.activeSymbol][-1].bidPrice1)+" "+"volume:"+str(volume))
    ...
    def runCatchOrder(self):
        ...
        while self.orderFlag:
            try:
                order = self.catchOrderQueue.get(block = True, timeout = 1)  # 获取事件的阻塞时间设为1秒
    ...
    def runDisposeOrder(self):
        while self.orderFlag:
            try:
                order = self.disposeOrderQueue.get(block = True, timeout = 1)  # 获取事件的阻塞时间设为1秒
    ...
    def onDisposeOrder(self,order):
        ...
        if (order.status=="全部成交" or order.status=="部分成交") and order.vtSymbol==self.activeSymbol and order.vtOrderID in self.active_passiveMap:
            ...
            if order.offset=="开仓":
                self.writeCtaLog("开仓"+"***"+str(order.vtOrderID)+":"+str(passiveID)+" "+str(self.floor_infoMap[floor]["miu"])+" "+str(self.floor_infoMap[floor]["gap"]))                        
                ...
                #可平仓量减去已经交易的交易量
                self.writeCtaLog("平仓"+"***"+str(order.vtOrderID)+":"+str(passiveID)+" "+str(self.floor_infoMap[floor]["miu"])+" "+str(self.floor_infoMap[floor]["gap"]))
        if order.status=="全部成交":
            if order.vtSymbol==self.activeSymbol and order.offset=="平仓"and order.vtOrderID in self.active_passiveMap and self.active_passiveMap[order.vtOrderID] in self.passive_floorMap:
                ...
                self.writeCtaLog("floor:%s可平仓量为%s"%(floor,self.floor_infoMap[floor]["volume"]))
    ...
    def onCatchOrder(self,order):
        if (order.status=="全部成交" or order.status=="部分成交") and  order.vtSymbol==self.passiveSymbol and order.vtOrderID in self.passive_countList:
            callback=self.offset_directionCatchOrderMap[(order.offset,order.direction)]
            callback(order)
            if order.status=="全部成交" and order.vtSymbol==self.passiveSymbol and order.vtOrderID in self.passive_countList:#防止restful怼过去进两遍拒单逻辑
                ...
                self.writeCtaLog("delete vtOrderID:%s from passive_countList"%(order.vtOrderID))
            # self.active_passiveMap[activeID]=passiveID
...
#             self.writeCtaLog("本地撤单")
                ...
                if bid_askSpread<self.gap_base:
                    if pos==0 and isEnter:
                        ...
                        if spread>self.miu+1/2*self.gap_base*floor:
                            ...
                            passiveID=self.buy(self.passiveSymbol,self.tickBufferDict[self.activeSymbol][-1].askPrice1-self.alpha-self.miu-gap_base*floor,base_unit,levelRate=10,priceType = PRICETYPE_LIMITPRICE)[0]        
                            if  passiveID:
                                self.passive_countList.append(passiveID)
                                
                                self.floor_infoMap[floor].update({"passiveID":passiveID,"miu":self.miu,"gap":gap_base,"volume":0,"pos":2,"spread":spread,"isAdd":False,"isNormal":False,"should_increase":0})
                                self.passive_floorMap[passiveID]=floor
                                self.passive_volume[passiveID]=0
                        elif spread<self.miu-1/2*self.gap_base*floor :

                            #限价单
                            passiveID=self.short(self.passiveSymbol,self.tickBufferDict[self.activeSymbol][-1].bidPrice1-self.alpha-self.miu+gap_base*floor,base_unit,levelRate=10,priceType = PRICETYPE_LIMITPRICE)[0]
                            if  passiveID:
                                self.passive_countList.append(passiveID)
                        ...
                        rev_floor=self.open_closeGap[floor]+self.floor_infoMap[floor]["should_increase"]

                        miu=self.floor_infoMap[floor]["miu"]
                        
                        volume=self.floor_infoMap[floor]["volume"]

                        closeID=self.sell(self.passiveSymbol,self.tickBufferDict[self.activeSymbol][-1].bidPrice1-self.alpha-miu-gap_base*rev_floor,volume,levelRate=10,priceType=PRICETYPE_LIMITPRICE)[0]
                        if  closeID:
                        ...
                        closeID=self.cover(self.passiveSymbol,self.tickBufferDict[self.activeSymbol][-1].askPrice1-self.alpha-miu+gap_base*rev_floor,volume,levelRate=10,priceType=PRICETYPE_LIMITPRICE)[0]
                        if  closeID:
                            passiveID=self.floor_infoMap[floor]["passiveID"]
                            self.close_openIdMap[closeID]=passiveID
    ...
    def onBar(self,bar):
        ...
        self.writeCtaLog("vtSymbol:%s datetime:%s"%(bar.vtSymbol,bar.datetime))
        ...
        if bar.vtSymbol==self.passiveSymbol:
            ...
            #观察到有1min同一品种推多个bar的，所以在这里进行处理
                for floor in list(self.floorOpenTimeMap.keys()):
                    if (bar.datetime-self.floorOpenTimeMap[floor]).total_seconds()>=180:
                        self.floor_infoMap[floor]["should_increase"]+=(floor-self.open_closeGap[floor])/8
                        self.floorOpenTimeMap[floor]=bar.datetime
                        rev_floor=self.open_closeGap[floor]
                        gap_base=self.floor_infoMap[floor]["gap"]
                        self.writeCtaLog("十五分钟到,此时floor:%s价差已增加:%s"%(floor,self.floor_infoMap[floor]["should_increase"]))
                        if (floor-rev_floor-self.floor_infoMap[floor]["should_increase"])<=2*gap_base:
                            ...
                            volume=self.floor_infoMap[floor]["volume"]    
                            if pos==-1:
                                closeID=self.cover(self.passiveSymbol,self.midPassiveBuffer[-1]*1.02,volume,levelRate=10,priceType=PRICETYPE_LIMITPRICE)[0]
                                self.close_openIdMap[closeID]=self.floor_infoMap[floor]["passiveID"]
                                self.floor_infoMap[floor]["pos"]=-2
                                self.passive_volume[self.floor_infoMap[floor]["passiveID"]]=0
                                self.passive_countList.append(closeID)
                            elif pos==1:
                                closeID=self.sell(self.passiveSymbol,self.midPassiveBuffer[-1]*0.98,volume,levelRate=10,priceType=PRICETYPE_LIMITPRICE)[0]
                                self.passive_volume[self.floor_infoMap[floor]["passiveID"]]=0
                                self.close_openIdMap[closeID]=self.floor_infoMap[floor]["passiveID"]
                                self.passive_countList.append(closeID)
                                self.floor_infoMap[floor]["pos"]=-3
                            self.writeCtaLog("移动止损过程中，发现profit不能cover手续费，故直接止损")    
                ...
                for order in list(self.unknowOrderIDMap.values()):
                    if order.vtSymbol==self.passiveSymbol and (bar.datetime-order.deliverTime).total_seconds()>120:
                        if order.offset=="平仓" and order.vtOrderID in self.close_openIdMap :#平仓订单撤销把pos和flag修改，让它恢复原样
                            ...
                            del self.passive_volume[passiveID]
                            del self.close_openIdMap[order.vtOrderID]
                        elif order.offset=="开仓" and self.floor_infoMap[self.passive_floorMap[order.vtOrderID]]["volume"]==0 and order.vtOrderID in self.passive_countList:#如果开仓的时候订单就没了，就直接把字典删除，让它恢复没有下单一样
        ...
        #                     print("市价追开空")
                            activeID=self.buy(self.activeSymbol,order.price,volume,priceType=PRICETYPE_MARKETPRICE,levelRate=10)[0]
                    
                            self.writeCtaLog("市价追开多"+str(self.midActiveBuffer[-1])+" "+"volume:"+str(volume))
                            
                        elif order.direction=="空" and order.offset=="开仓":
        #                     print("市价追开多")
                            activeID=self.short(self.activeSymbol,order.price,volume,priceType=PRICETYPE_MARKETPRICE,levelRate=10)[0]
                            
                            self.writeCtaLog("市价追开多"+str(self.midActiveBuffer[-1])+" "+"volume:"+str(volume))
                            
                        elif order.direction=="多"  and order.offset=="平仓":
        #                     print("市价追平多")
                            activeID=self.cover(self.activeSymbol,order.price,volume,priceType=PRICETYPE_MARKETPRICE,levelRate=10)[0]
                            
                            self.writeCtaLog("市价追平多"+str(self.midActiveBuffer[-1])+" "+"volume:"+str(volume))
                            
                        elif order.direction=="空" and order.offset=="平仓":
        #                     print("市价追平空")
                            activeID=self.sell(self.activeSymbol,order.price,volume,priceType=PRICETYPE_MARKETPRICE,levelRate=10)[0]
                            self.writeCtaLog("市价追平空"+str(self.midActiveBuffer[-1])+" "+"volume:"+str(volume))
                            
                        if activeID:
                            self.active_passiveMap[activeID]=passiveID
                        self.writeCtaLog(u'未知重新发市价单：%s,%s,%s,%s'%(order.vtSymbol,order.direction,order.offset,order.totalVolume))
                        del self.unknowOrderIDMap[order.vtOrderID]
        ...
        if len(self.barSpreadBuffer)%15==0:
            ...
            self.writeCtaLog("十五分钟，时间到改变miu:%s和gap:%s"%(self.miu,self.gap_base))
        if len(self.barSpreadBuffer)%15==0:
            del self.barBufferDict[self.passiveSymbol][:-240]
            del self.barBufferDict[self.activeSymbol][:-240]
            del self.activeDateTime[:-240]
    ...
    def onhfBar(self,bar):
        ...
        if bar.vtSymbol==self.activeSymbol:
            ...
            self.contin15MinCloseReturn=max(1,abs((bar.close-self.init15MinClose)/self.init15MinClose)*5+0.8)    
            self.writeCtaLog("this 10s return is %s"%abs((bar.close-self.init15MinClose)/self.init15MinClose))
            if self.contin15MinCloseReturn >1:
                self.writeCtaLog("波幅较大，动态更新gap_base")
            
            if bar.vtSymbol==self.activeSymbol:
                self.writeCtaLog(bar.vtSymbol+":"+"longTimeRate=%s"%self.longTickRate)
                self.writeCtaLog(bar.vtSymbol+":"+"shortTimeRate=%s"%self.shortTickRate)
                self.writeCtaLog(bar.vtSymbol+":"+"longPriceVol=%s"%self.longPriceVol)
                self.writeCtaLog(bar.vtSymbol+":"+"shortPriceVol=%s"%self.shortPriceVol)
                for floor in self.floors:
                    ...
                    if self.shortTickRate>=out_percentage*self.longTickRate:
                        ...
                        self.writeCtaLog(str(floor)+"isNormal has been setted to False")
                    else:
                    ...
                    if self.shortPriceVol>=in_percentage*1.2*self.longPriceVol or self.spreadBuffer[-1]>self.miu+self.gap_base*self.last_floor*1.2 or self.spreadBuffer[-1]<self.miu-self.gap_base*self.last_floor*1.2:
                        ...
                        self.writeCtaLog(str(floor)+"isEnter has been setted to False")
                    else:
    ...
    #                     print("市价追开多")
                        ...
                        self.writeCtaLog("市价追开多"+str(self.tickBufferDict[self.activeSymbol][-1].bidPrice1)+" "+"volume:"+str(volume))
...
class OutEvent(object):
    ...
    def __init__(self,activeSymbol,passiveSymbol):
        ...
        self.tickBufferDict={symbol:[] for symbol in [activeSymbol,passiveSymbol]}
        # 事件引擎开关
        self.__active = False
        self.shortTimeCountBufferDict={symbol:[] for symbol in [activeSymbol,passiveSymbol]}
        self.longTimeCountBufferDict={symbol:[] for symbol in [activeSymbol,passiveSymbol]}
        # 事件处理线程
        ...
        if tick.vtSymbol==self.activeSymbol:
            ...
            #保证lastPrice价格是不同的，此时tick才存进list里
                ...
                if (tick.datetime-firstShortTick.datetime).total_seconds()>self.__shortTerm:
                    self.__shortTickRate=len(self.shortTimeCountBufferDict[tick.vtSymbol])/self.__shortTerm
                    shortLastPriceArray=np.array(list(map(lambda x:x.lastPrice, self.shortTimeCountBufferDict[tick.vtSymbol])))

                    self.__shortPriceVol=np.std((shortLastPriceArray[1:]-shortLastPriceArray[:-1])/shortLastPriceArray[:-1])
                    self.shortTimeCountBufferDict[tick.vtSymbol]=[]
                    print("成功更新短期tick速率和price波动率")
                    longLastPriceArray=np.array(list(map(lambda x:x.lastPrice,self.longTimeCountBufferDict[tick.vtSymbol])))
                    self.__longTickRate=len(self.longTimeCountBufferDict[tick.vtSymbol])/self.__longTerm
                    self.__longPriceVol=np.std((longLastPriceArray[1:]-longLastPriceArray[:-1])/longLastPriceArray[:-1])
            ...
            if tick.datetime>self.tickBufferDict[tick.vtSymbol][-1].datetime:
                ...
                if len(self.shortTimeCountBufferDict[tick.vtSymbol])==0:
                    ...
                    self.shortTimeCountBufferDict[tick.vtSymbol].insert(-2,tick)
                    self.longTimeCountBufferDict[tick.vtSymbol].insert(-2,tick)
